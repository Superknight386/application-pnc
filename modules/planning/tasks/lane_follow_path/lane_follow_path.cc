...
apollo::common::Status LaneFollowPath::Process(
  ...
  const auto blocking_obstacle_num = reference_line_info_->path_decision()->obstacles().Items().size();

  if(blocking_obstacle_num>8)
  {
    const auto blocking_obstacle_Items=reference_line_info_->path_decision()->obstacles().Items();
    for(auto obstacle:blocking_obstacle_Items)
    {
      reference_line->AddSpeedLimit(obstacle->PerceptionSLBoundary().start_s()-5,obstacle->PerceptionSLBoundary().end_s()+5,5);
    }
  }
  //
