...
Status PiecewiseJerkSpeedOptimizer::Process(const PathData& path_data,
  ...
  //
  const auto x_position = init_point.path_point().x();

  if(x_position>423345&&x_position<423558)
  {
        penalty_dx.push_back(std::fabs(path_point.kappa()) *
                         20);
        AINFO<<"减速中:"<<x_position;
  }
  //
  else
  {
  ...
  if(x_position>423344&&x_position<423558)
  {
    piecewise_jerk_problem.set_x_ref(100, std::move(x_ref));
    AINFO<<"减速中:"<<x_position;
  }
  //
  else
