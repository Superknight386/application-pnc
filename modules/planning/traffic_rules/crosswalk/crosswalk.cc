...
Status Crosswalk::ApplyRule(Frame* const frame,
  ...
  for (const auto&crosswalk_overlap : crosswalk_overlaps_)
  {
    reference_line->AddSpeedLimit(crosswalk_overlap->start_s -2, crosswalk_overlap->end_s + 2, 4.9);
  }
...
void Crosswalk::MakeDecisions(Frame* const frame,
  ...
  for (auto crosswalk_to_stop : crosswalks_to_stop) {
    ...
    util::BuildStopDecision(
        ...
        2.3, StopReasonCode::STOP_REASON_CROSSWALK,
  ...
  if (firsts_crosswalk_to_stop) {
    ...
    // std::string_virtual_obstacle_id = CROSSWALK_VO_ID_PREFIX + crosswalk_overlap->object_id;
    // if(injector_->vehicle_state()->linear_velocity()>4)
    // {
    //   util::BuildStopDecision(
    //     virtual_obstacle_id,crosswalk_overlap->start_s,config_.stop_distance()-4,StopReasonCode::STOP_REASON_CROSSWALK,
    //     crosswalk_to_stop.second,Getname(),frame,reference_line_info
    //   );
    // }
    // else
    // {
    //   util::BuildStopDecision(
    //     virtual_obstacle_id,crosswalk_overlap->start_s,config_.stop_distance(),StopReasonCode::STOP_REASON_CROSSWALK,
    //     crosswalk_to_stop.second,Getname(),frame,reference_line_info
    //   );
    // }
...
bool Crosswalk::CheckStopForObstacle(
  ...
  if(obstacle_l_distance<config_.stop_strict_l_distance()) stop = true;
  return stop;
}
